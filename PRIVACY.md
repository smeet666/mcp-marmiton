...
`mcp-marmiton` is a read-only client for [Marmiton](https://www.marmiton.org). It runs on your
...
**One host is contacted: `www.marmiton.org`.** Nothing else.
...
| A `User-Agent`         | `mcp-marmiton/<version> (+https://github.com/smeet666/mcp-marmiton)`, so the site can reach a person about the traffic it receives. |
| Your IP address        | Sent by your network to any host you contact, as with any web request.                                                              |

Your requests reach Marmiton. What is done with them there is governed by that site's own privacy policy, which this project does not control.
...
becomes of them. `MARMITON_LOG_LEVEL` governs how much is written and defaults to `error`. These lines stay on your machine.
...
Open an issue on [the repository](https://github.com/smeet666/mcp-marmiton/issues). For something exploitable,
...
`mcp-marmiton` est un client en lecture seule pour [Marmiton](https://www.marmiton.org). Il
...
**Un seul hôte est joint : `www.marmiton.org`.** Rien d'autre.
...
| Un `User-Agent`   | `mcp-marmiton/<version> (+https://github.com/smeet666/mcp-marmiton)`, pour que le site puisse joindre une personne au sujet du trafic qu'il reçoit. |
| Votre adresse IP  | Transmise par votre réseau à tout hôte que vous joignez, comme pour n'importe quelle requête web.                                                   |

Vos requêtes atteignent Marmiton. Ce qui en est fait là-bas relève de la politique de confidentialité de ce site, que ce projet ne contrôle pas.
...
qu'ils deviennent. `MARMITON_LOG_LEVEL` règle leur quantité et vaut `error` par défaut. Ces lignes restent sur votre machine.
...
Ouvrez une issue sur [le dépôt](https://github.com/smeet666/mcp-marmiton/issues). Pour quelque chose
