...
1. Bump the version in **three** places, which must stay in step:
   ...
   - `src/version.ts`, which feeds the User-Agent sent to MARMITON
...
```bash
npm view mcp-marmiton version
npx -y mcp-marmiton          # should start and wait on stdin
```
