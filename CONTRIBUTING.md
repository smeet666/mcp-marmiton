...
Open an issue: <https://github.com/smeet666/mcp-marmiton/issues>
...
a rescaled quantity no cook would write, such as a third of an egg is a real bug, and it is only visible to
...
It reads Marmiton and returns what it reads. It does not write anything back, it
...
- **Every result carries its source.** Content that came from Marmiton goes out
  with a link to the page it came from, and the text block keeps that link even
  when it has to be shortened.
- **The server paces itself.** Marmiton is someone else's site. The minimum
...
`MARMITON_LIVE=1 npm run test:live`, which does: it makes one request per route
against https://www.marmiton.org and checks the fields the parsers depend on. Run it if you
...
The tools are `search_recipes`, `get_recipe` and `scale_ingredients`. A new tool is a bigger conversation than a new field on
