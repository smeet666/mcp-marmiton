...
export function parseIsoDuration(value: unknown): number | null {
  ...
  if (total === 0 && !minutes && !hours && !days && !seconds) return null;
...
export function parseYield(value: unknown): ParsedYield {
  // schema.org allows recipeYield as a number, a string, or an array of either.
  const text =
    typeof value === "number" && Number.isFinite(value)
      ...
      : typeof value === "string"
        ...
        : Array.isArray(value)
          ? String(value[0] ?? "").trim()
          : "";
  if (!text) return { count: null, unit: null, text: "" };
